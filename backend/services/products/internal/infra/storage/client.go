...
var ErrBucketAlreadyExists = e.NewErrorFrom(e.ErrConflict).SetMessage("bucket already exists")
...
type Client interface {
	...
	UploadFileByReader(ctx context.Context, bucket BucketName, fileName string, r io.Reader, metadata map[string]string) (key string, hash string, mimeType string, err error)
	UploadFileByReaderWithMultipart(ctx context.Context, bucket BucketName, fileName string, r io.Reader, metadata map[string]string) (key string, hash string, mimeType string, err error)
	UploadFileByBytes(ctx context.Context, bucket BucketName, fileName string, data []byte, metadata map[string]string) (key string, hash string, mimeType string, err error)
}
